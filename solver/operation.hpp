...
struct Operation {
  inline constexpr Operation() : data(0){}

  inline constexpr Operation(const unsigned int x) : data(x){}

  inline constexpr Operation(const int kata, const int y, const int x, const Dir dir) : data(0){
  ...
  friend std::istream &operator>>(std::istream &is, Operation &ops){
    int kata,y,x,dir;
    is >> kata >> y >> x >> dir;
